...
def save_to_csv(all_cars, filename="pakwheels_data.csv"):
    ...
    columns = ["brand", "model", "year", "mileage_km", "fuel_type",
               "transmission", "engine_cc", "city", "price_pkr"]
    
    df = pd.DataFrame(all_cars, columns=columns)
    df.to_csv(f"./laptop_price_predictor_project/data/{filename}", index=False)
